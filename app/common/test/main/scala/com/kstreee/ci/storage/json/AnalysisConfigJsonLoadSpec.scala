...
class AnalysisConfigJsonLoadSpec(implicit ee: ExecutionEnv) extends Specification {
  "analysis reads" should {
    "valid analysis json" in {
      // PylintAnalyzer
      val analyzerName = AnalyzerConfigJsonLoad.pylintName
      val analysisCmd = Seq("1", "2", "3")
      val reportFormat = "json"
      // CLICoordinator
      val coordinatorName = CoordinatorConfigJsonLoad.cliName
      val timeoutSeconds = 123
      // FileSystemSymbolic
      val sourcecodeLoaderName = SourcecodeLoaderConfigJsonLoad.fileSystemName
      val sourcePath = "TEST_SOURCE_PATH"
      // CLIReporter
      val reporterName = ReporterConfigJsonLoad.cliPlainName
      val json = s"""
               |{
               |  "analyzer": {
               |    "name": "$analyzerName",
      ...
      (for {
        result <- AnalysisConfigJsonLoad.load(Json.parse(json))
      } yield {
