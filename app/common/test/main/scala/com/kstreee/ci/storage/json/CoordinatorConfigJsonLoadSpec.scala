...
class CoordinatorConfigJsonLoadSpec(implicit ee: ExecutionEnv) extends Specification {
  "cli coordinator reads" should {
    "valid cli coordinator json" in {
      val name = CoordinatorConfigJsonLoad.cliName
      ...
      """.stripMargin
      (for {
        result <- CoordinatorConfigJsonLoad.load(Json.parse(json))
      } yield {
  ...
  "file coordinator reads" should {
    "valid cli coordinator json" in {
      val name = CoordinatorConfigJsonLoad.fileName
      val reportPath = "ABCD"
      val json =
        s"""
           |{
