...
class ReporterConfigJsonLoadSpec(implicit ee: ExecutionEnv) extends Specification {
  "cli json reporter reads" should {
    "valid cli json reporter json" in {
      val name = ReporterConfigJsonLoad.cliJsonName
      val json = s"""
                    |{
                    |  "name": "$name"
                    |}
      """.stripMargin
      (for {
        result <- ReporterConfigJsonLoad.load(Json.parse(json))
      } yield {
  ...
  "cli plain reporter reads" should {
    "valid cli plain reporter json" in {
      val name = ReporterConfigJsonLoad.cliPlainName
      val json = s"""
                    |{
                    |  "name": "$name"
                    |}
  ...
  "github issue reporter reads" should {
    "valid github issue reporter json" in {
      val name = ReporterConfigJsonLoad.githubIssueName
